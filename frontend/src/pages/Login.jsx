...
/**
 * Login component that handles user authentication with email and OTP
 */
const Login = () => {
  ...
  const sendOTP = async (event) => {
    ...
    try {
      await axios.post("/election/generateOtp", { email });
  ...
  /**
   * Handles user login with email and OTP
   */
  const loginHandler = async (event) => {
    ...
    try {
      ...
      if (response && response.data && response.data.user) {
        ...
        notify("Login failed - Invalid response from server", "error");
      }
    } catch (err) {
      notify(err.response?.data?.errMessage || "Login failed", "error");
    } finally {
      setLoading(false);
    }
  };
